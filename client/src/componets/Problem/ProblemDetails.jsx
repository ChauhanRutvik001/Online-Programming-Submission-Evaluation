...
import { 
  ArrowLeft, Calendar, Users, CheckCircle, School, Book, 
  Clock, Info, ChevronRight, AlertCircle, Award, Search,
  ChevronLeft, ChevronUp, Filter
} from "lucide-react";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        try {
          const facultyResponse = await axiosInstance.get("/faculty/my-batches");
          if (facultyResponse.data.success) {
            // If this succeeds, the user is a faculty member
            const batchIds = facultyResponse.data.batches?.map(b => b._id) || [];
        ...
        const response = await axiosInstance.get(`/problems/${problemId}/details`);
        setProblem(response.data.problem);
        console.log("Problem details:", response.data.problem);
        // If we have faculty batch IDs, filter the problem's batches
        if (facultyBatchIds.length > 0 && response.data.problem?.batchDueDates) {
    ...
    try {
      ...
      if (response.data.batches) {
        ...
        if (facultyBatchIds.length > 0) {
          batchesToShow = batchesToShow.filter(
            bd => facultyBatchIds.includes(bd.batch?._id || bd.batch)
          );
          setTotalBatchPages(Math.ceil(batchesToShow.length / BATCHES_PER_PAGE));
        } else {
          setTotalBatchPages(response.data.pagination.totalPages);
        }
        
        setFilteredBatches(batchesToShow);
        setTotalBatches(facultyBatchIds.length > 0 ? batchesToShow.length : response.data.pagination.totalBatches);
      } 
      // If we get full problem response (backward compatibility)
        ...
        const filtered = searchTerm 
          ? allBatches.filter(b => 
              (b.batch?.name && b.batch.name.toLowerCase().includes(searchTerm.toLowerCase())) ||
              (b.batch?.subject && b.batch.subject.toLowerCase().includes(searchTerm.toLowerCase())))
          : allBatches;
              ...
              <div className="p-6 border-b border-gray-700">
                ...
                <div className="flex flex-wrap gap-4 text-sm text-gray-300">
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    Created by: {problem.createdBy?.name || problem.createdBy?.username || "Unknown"}
                  </span>
                  <span className="flex items-center gap-1">
                    <Award className="w-4 h-4 text-blue-400" />
                    Total marks: {problem.totalMarks || 0}
                  </span>
                </div>
                {problem.tags && problem.tags.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {problem.tags.map((tag, index) => (
                      <span 
                        key={index}
                        className="bg-blue-900/40 text-blue-300 px-2 py-1 rounded text-xs font-medium"
                      >
                        {tag}
                      </span>
                    ))}
                    ...
                    {problem.sampleIO.map((sample, index) => (
                      <div key={index} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-gray-900 p-4 rounded-lg">
                          <h3 className="text-sm font-medium text-gray-400 mb-2">Sample Input {index + 1}</h3>
                          <pre className="text-gray-200 whitespace-pre-wrap font-mono text-sm bg-gray-950 p-3 rounded border border-gray-800">
                            {sample.input}
                          </pre>
                        </div>
                        <div className="bg-gray-900 p-4 rounded-lg">
                          <h3 className="text-sm font-medium text-gray-400 mb-2">Sample Output {index + 1}</h3>
                          <pre className="text-gray-200 whitespace-pre-wrap font-mono text-sm bg-gray-950 p-3 rounded border border-gray-800">
                            {sample.output}
                          </pre>
                        </div>
                ...
                {filteredBatches.length > 0 ? (
                  <>
                    <div className="space-y-3">
                      {filteredBatches.map((batchDue) => {
                        const timeRemaining = batchDue.dueDate ? getTimeRemaining(batchDue.dueDate) : null;
                        ...
                        return (
                          <div 
                            ...
                            onClick={() => navigate(`/problems/${problemId}/${batchDue.batch?._id || batchDue.batch}`)}
                          >
                            ...
                            <div className="flex flex-col gap-2 text-sm text-gray-300">
                              {batchDue.batch?.subject && (
                                <div className="flex items-center gap-1">
                                  ...
                                  <span>{batchDue.batch.subject}</span>
                                </div>
                              )}
                              ...
                              {batchDue.batch?.students && (
                                <div className="flex items-center gap-1">
                                  ...
                                  <span>{batchDue.batch.students.length} students</span>
                                </div>
                              )}
                              ...
                              {batchDue.dueDate && (
                                <>
                                  <div className="flex items-center gap-1">
                                    ...
                                    <span>Due: {formatDate(batchDue.dueDate)}</span>
                                  </div>
                                  
                                  {timeRemaining && (
                                    <div className="flex items-center gap-1">
                                      <Clock size={14} className={isPastDue ? "text-red-400" : "text-yellow-400"} />
                                      <span className={isPastDue ? "text-red-400" : "text-yellow-400"}>
                                        {timeRemaining}
                                      </span>
                                    </div>
                                  )}
                            ...
                            <div className="flex justify-end mt-3">
                              <button 
                                ...
                                onClick={(e) => {
                                  ...
                                  navigate(`/problems/${problemId}/${batchDue.batch?._id || batchDue.batch}`);
                                }}
                              >
                  ...
                  <div className="bg-gray-750 rounded-lg p-6 text-center border border-gray-700">
                    ...
                    <p className="text-gray-400">
                      {batchSearchTerm ? "No batches match your search." : "This problem is not assigned to any of your  batches yet."}
                    </p>
                  </div>
                )}
              </div>
