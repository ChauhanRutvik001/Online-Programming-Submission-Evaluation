...
const Header = () => {
  ...
  const location = useLocation();  const [isAdmin, setIsAdmin] = useState(false);
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [isScreenSmall, setIsScreenSmall] = useState(false);
  const [isMediumScreen, setIsMediumScreen] = useState(false);
  const [isOnMakeContest, setIsOnMakeContest] = useState(false);
  const [isUserMenuOpen, setIsUserMenuOpen] = useState(false);  const [isNotificationOpen, setIsNotificationOpen] = useState(false);
  ...
  return (
    <motion.nav
      initial={{ y: -100 }}
      animate={{ y: 0 }}
      ref={headerRef}      className={`z-50 w-full fixed transition-all duration-300 ${
        scrolled 
        ...
        scrolled 
          ...
          >
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="relative" ref={notificationMenuRef}>                <NotificationIcon 
                ...
                {/* Desktop-style Notification Dropdown for Tablet View */}                <AnimatePresence>
                  {isNotificationOpen && (                    <div className="absolute right-0 mt-2 z-50">
                      <NotificationList 
              ...
              <div className="hidden sm:flex items-center space-x-0.5 sm:space-x-1 md:space-x-1 lg:space-x-2 xl:space-x-3">
                {navLinks.map((link) => (
                  <Link
                    ...
                    to={link.path}                    className={`flex items-center space-x-0.5 sm:space-x-1 px-1 sm:px-1.5 md:px-2 lg:px-2 xl:px-3 py-1.5 xl:py-2 rounded-md text-xs sm:text-xs md:text-xs lg:text-sm xl:text-sm font-medium transition-all duration-200 ${
                      isActive(link.path)
                ...
                <div className="relative" ref={notificationMenuRef}>
                  ...
                  >
                    ...
                    {isNotificationOpen && (                      <div className="absolute right-0 mt-2">
                        <NotificationList 
                          onClose={() => setIsNotificationOpen(false)}
                          onViewAll={() => {
                            setIsNotificationOpen(false);
          ...
          >
            <div className="px-3 sm:px-4 pt-2 pb-3 space-y-1 max-h-screen overflow-y-auto">              {navLinks.map((link) => (
              ...
              <div className="relative">
                ...
                {/* Mobile Notification Dropdown */}                <AnimatePresence>
                  {isNotificationOpen && (                    <motion.div
                      initial={{ opacity: 0, height: 0 }}
                      animate={{ opacity: 1, height: "auto" }}
                      exit={{ opacity: 0, height: 0 }}
                      transition={{ duration: 0.2 }}
